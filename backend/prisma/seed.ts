...
async function main() {
  ...
  if (!adminEmail) {
    throw new Error(
      'ADMIN_EMAIL environment variable is required. Please set it in your .env file.'
    );
  }

  if (!adminPassword) {
    throw new Error(
      'ADMIN_PASSWORD environment variable is required. Please set it in your .env file.'
    );
  }
