...
export class ShareController {
  constructor(
    ...
    private createFolderShareUseCase: CreateFolderShareUseCase,
   ...
   * Share a folder internally or create a public link
   */
  async createFolderShare(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        ResponseUtil.unauthorized(res);
        return;
      ...
      if (!id || typeof id !== 'string') {
        ResponseUtil.validationError(res, 'Invalid folder ID');
        return;
      }
      const { type, permission, email, expiresInSeconds } = req.body;
      ...
      const result = await this.createFolderShareUseCase.execute({
        folderId: id,
        ownerId: req.user.id,
        type: type,
        permission: permission,
      ...
      let responseData: any = result;
      if (result.type === ShareType.PUBLIC_LINK && result.token) {
        responseData = { ...result, link: `/share/folder/${result.token}` };
      }

      ResponseUtil.success(res, responseData, 'Folder shared successfully');
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Share failed';
      ResponseUtil.error(res, 'SHARE_ERROR', message);
    }
  ...
  /**
   * GET /api/share/:token
   * Access a shared file or folder via public link
   */
  async getSharedItem(req: Request, res: Response): Promise<void> {
    try {
      ...
      try {
        ...
        // If file not found, try folder
        try {
          const { share, folder, isExpired } = await this.getSharedFolderUseCase.execute(token);
